...
class Decoder(nn.Module):
    ...
    def inference_noattention(self, memory, f0s, attention_map):  ###pitchtron에서 가져옴
        """ Decoder inference
        PARAMS
        ------
        memory: Encoder outputs
...
class Tacotron2(nn.Module):
    ...
    '''def parse_batch(self,batch):   ###위에서 class 안으로 추가
        text_padded, input_lengths, mel_padded, gate_padded, \
        output_lengths, speaker_ids, f0_padded, input_ids, attention_mask, speaking_emotion = batch
        text_padded = to_gpu(text_padded).long()
        input_lengths = to_gpu(input_lengths).long()
    ...
    def inference(self, inputs):
        self.emotion_distil_bert = EmotionDistilBert.from_pretrained('/content/drive/MyDrive/emotion-tts/emotion_tts/checkpoint/DistilBERT.zip (Unzipped Files)').cuda().eval() #여기수정
        inputs, input_lengths, targets, max_len, \
            output_lengths, speaker_ids, f0s, input_ids, attention_mask, emotion_supervision = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
    ...
    def inference_override(self, inputs, emotion_idx):
        self.emotion_distil_bert = EmotionDistilBert.from_pretrained(
            '/content/drive/MyDrive/emotion-tts/emotion_tts/checkpoint/DistilBERT.zip (Unzipped Files)').cuda().eval()
        inputs, input_lengths, targets, max_len, \
        output_lengths, speaker_ids, f0s, input_ids, attention_mask, emotion_supervision = inputs
        input_lengths, output_lengths = input_lengths.data, output_lengths.data
        embedded_inputs = self.embedding(inputs).transpose(1, 2)
    ...
    def inference_noattention(self, inputs):   ###pitchtron 에서 추가
